...
msleep %>% 
  select(order,vore) %>% 
  filter(vore != "carni", vore !="herbi", vore != "insecti")
```
...
```{r}
msleep %>% 
  summarize(mean_mammal = mean(sleep_total),
            max_mammal = max(sleep_total),
            min_mammal = min(sleep_total),
            total = n())
```
...
##### NA means there is no data there. NA's are included in the summarize function.
