...
```r
mammals_limit <- select(mammals, "family", "genus", "species", "mass", "gestation")
mammals_limit
```
...
```r
filter(mammals_limit, family=="Bovidae" & mass>450000)
```
...
```r
mammals_genera <- select(mammals, "genus", "mass", "gestation", "newborn")
primate_genera <- filter(mammals_genera, genus == "Lophocebus"| genus == "Erythrocebus"| genus == "Macaca")
primate_genera
```
...
```r
filter(primate_genera, newborn == min(newborn))
```
...
```
#### The genus Macaca has the smallest newborn size
